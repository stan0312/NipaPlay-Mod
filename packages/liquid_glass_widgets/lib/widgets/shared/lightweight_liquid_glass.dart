...
import 'liquid_shape_shader_geometry.dart';
...
class _RenderLightweightGlass extends RenderProxyBox {
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (blurSigma > 0 && !_skipBlur) {
      ...
      // Keep the same compositor layer across route/physics animation frames.
      // Recreating it here used to discard the engine layer on every repaint,
      // forcing Android to repeatedly allocate and submit the blur pass.
      layer ??= BackdropFilterLayer();
      layer!.filter = filter;
      context.pushLayer(layer!, _paintGlassContent, offset);
      assert(() {
  ...
  void _updateShaderUniforms(Size size, Offset physicalOrigin,
    ...
    // For vertical rounded shapes, write -1.0 to signal asymmetric mode.
    // Slots 24-27 (uData6) carry the four per-corner radii in that case.
    double? cornerRadius;
    double topLeftR = 0.0;
    ...
    if (_shape is LiquidVerticalRoundedSuperellipse ||
        _shape is LiquidVerticalRoundedRectangle) {
      ...
      final (topRadius, bottomRadius) = switch (_shape) {
      ...
      topLeftR = topRadius.clamp(0.0, maxTop);
      topRightR = topRadius.clamp(0.0, maxTop);
      bottomRightR = bottomRadius.clamp(0.0, maxBot);
      bottomLeftR = bottomRadius.clamp(0.0, maxBot);
      isAsymmetric = true;
    } else {
      cornerRadius = resolveLiquidShapeCornerRadius(_shape, size);
    }
